###########################################################################
# 4. 모델 학습 - Decision Tree, automl_modeltraining_decisiontree.py
###########################################################################
...
class Automl_Modeltraining_DecisionTree:
    isLocal = False
    ...
    def recognition(self, parameterList):
        ...
        # (4) 결정 나무 적합
        clf = DecisionTreeClassifier(random_state=123, max_leaf_nodes=11, min_samples_leaf=5)
        ...
        #     - 결정나무 트리 - 서버에 저장하기    
        insertDocSn = docSn     
        training_gubun = '11'    # 학습 구분 : 11 - (Decision Tree)
                        
        dataconfirmFilePath = r"C:/dev_project/auto_ml/textimagefile"
        imageFileNm =  '/DecisionImage10_' + insertDocSn + '.png'
        imageFile   =  'DecisionImage10_' + insertDocSn + '.png'
        plt.savefig(dataconfirmFilePath + imageFileNm)                
                        
        # 1)정답 데이터 : y_test, 2)예측 결과 : y_pred
        #print(y_test)
        #print(y_pred)
        ...
        #    (6-2) 정확도 계산하기
        accu_clf = RandomForestClassifier(random_state=123, min_samples_leaf=5, max_features=11)
        ...
        # (7-2) Decision Tree > ROC 그래프 - 서버에 저장하기   
        image_gubun = '10'       # 이미지 구분
        
        dataconfirmFilePath = r"C:/dev_project/aiblue_ocr/textimagefile"
        imageFileNm =  '/DecisionTree10_' + insertDocSn + '.png'
        imageFile   =  'DecisionTree10_' + insertDocSn + '.png'
        plt.savefig(dataconfirmFilePath + imageFileNm)
                
        #       Decision Tree > ROC 그래프 - 서버에 저장하기        
        dataconfirmFilePath = r"C:/dev_project/auto_ml/textimagefile"
        imageFileNm =  '/DecisionTree10_' + insertDocSn + '.png'
        imageFile   =  'DecisionTree10_' + insertDocSn + '.png'
        plt.savefig(dataconfirmFilePath + imageFileNm)

        #      Decision Tree > ROC 그래프 이미지 - 테이블(TB_AUTOML_TRAINING_IMAGE)에 저장하기      
        file_nm   = imageFile
        ...
        # (7-3) Decision Tree > Confusion Matrix - 매트릭스 그리기 
        ...
        #       Decision Tree > Confusion Matrix - 서버에 저장하기   
        image_gubun = 20   # 이미지 구분
             
        dataconfirmFilePath = r"C:/dev_project/aiblue_ocr/textimagefile"
        imageFileNm =  '/DecisionTree20_' + insertDocSn + '.png'
        imageFile   =  'DecisionTree20_' + insertDocSn + '.png'
        plt.savefig(dataconfirmFilePath + imageFileNm)       
        ...
        #      Decision Tree > Confusion Matrix 이미지 - 테이블(TB_AUTOML_TRAINING_IMAGE)에 저장하기 
        file_nm   = imageFile
        file_path = dataconfirmFilePath
        file_full_path = dataconfirmFilePath + '/' + imageFile
...
if __name__ == '__main__':
    Automl_Modeltraining_DecisionTree = Automl_Modeltraining_DecisionTree()
    parameterList = {}
    ...
    Automl_Modeltraining_DecisionTree.recognition(parameterList)