###########################################################################
# 4. 모델 학습 - Random Forest, automl_modeltraining_randomforest.py
###########################################################################
...
class Automl_Modeltraining_RandomForest:
    isLocal = False
    ...
    def recognition(self, parameterList):
        ...
        #    (6-2) 정확도 계산하기
        accu_clf = RandomForestClassifier(random_state=123, n_estimators=7,
                                          min_samples_leaf=3, max_features=10, n_jobs=-1)
        ...
        insertDocSn = docSn     
        training_gubun = '12'    # 학습 구분 : 12 - (Random Forest)
        ...
        # (7-2) Random Forest > ROC 그래프 - 서버에 저장하기  
        image_gubun = '10'       # 이미지 구분
        
        dataconfirmFilePath = r"C:/dev_project/aiblue_ocr/textimagefile"
        imageFileNm =  '/RandomForest10_' + insertDocSn + '.png'
        imageFile   =  'RandomForest10_' + insertDocSn + '.png'
        plt.savefig(dataconfirmFilePath + imageFileNm)
                
        #       Random Forest > ROC 그래프 - 서버에 저장하기      
        dataconfirmFilePath = r"C:/dev_project/auto_ml/textimagefile"
        imageFileNm =  '/RandomForest10_' + insertDocSn + '.png'
        imageFile   =  'RandomForest10_' + insertDocSn + '.png'
        plt.savefig(dataconfirmFilePath + imageFileNm)
        
        #       Random Forest > ROC 그래프 이미지 - 테이블(TB_AUTOML_TRAINING_IMAGE)에 저장하기     
        file_nm   = imageFile
        ...
        # (7-3) RandomForest > Confusion Matrix - 매트릭스 그리기 
        ...
        #       RandomForest > Confusion Matrix - 서버에 저장하기   
        image_gubun = 20   # 이미지 구분
             
        dataconfirmFilePath = r"C:/dev_project/aiblue_ocr/textimagefile"
        imageFileNm =  '/RandomForest20_' + insertDocSn + '.png'
        imageFile   =  'RandomForest20_' + insertDocSn + '.png'
        plt.savefig(dataconfirmFilePath + imageFileNm)       
        ...
        #      RandomForest > Confusion Matrix 이미지 - 테이블(TB_AUTOML_TRAINING_IMAGE)에 저장하기 
        file_nm   = imageFile
        file_path = dataconfirmFilePath
        file_full_path = dataconfirmFilePath + '/' + imageFile
...
if __name__ == '__main__':
    Automl_Modeltraining_RandomForest = Automl_Modeltraining_RandomForest()
    parameterList = {}
    ...
    Automl_Modeltraining_RandomForest.recognition(parameterList)