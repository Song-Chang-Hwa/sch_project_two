...
@Component({
    selector: 'app-mlmodelevaluation',
    templateUrl: './mlmodelevaluation.component.html',
    styleUrls: ['./mlmodelevaluation.component.scss'],
    encapsulation: ViewEncapsulation.None,
})

// 모델 평가
export class MlmodelevaluationComponent implements OnInit, OnDestroy {
    ...
    onSearchOcrDocMstList(pageNo: any) {
        ...
        this.params = this.cf.toHttpParams({
            ...
            PAGESIZE: 10,
            FUCTION_GUBUN: '30', // 처리상태 코드 : 30(모델 확정 )
        });
                    ...
                    if (resultData.data.length > 0) {
                        ...
                        $.each(resultData.data, (index: any, item: any) => {
                            ...
                            tmp +=
                                ...
                                '                    </td>                                  ';
                            tmp +=
                                '<td><a href="javascript:void(0);" onClick="window.mlmodelevaluation.onSelectOcrDocDetail(' +
                ...
                if (resultData.code === 200) {
                    if (resultData.data.length > 0) {
                        ...
                        $.each(resultData.data, (index: any, item: any) => {
                            ...
                            tmp +=
                                '                    <td class="right">                      ' +
                                item.MODELCONFIRM_DTTM +
                                '                    </td>                                    ';
                            tmp +=
                                '<td><a href="javascript:void(0);" onClick="window.mlmodelevaluation.onDisplayTrainingResult(' +
                                // tslint:disable-next-line: prettier
                                '\'' +
                                i +
                                // tslint:disable-next-line: prettier
                                ...
                                '\');" class="btn btn-primary btn-xs btn-outline btn-fit">선택</a></td>';
                            tmp +=
                                '<td><a href="javascript:void(0);" onClick="window.mlmodelevaluation.onUpdateTrainingResult(' +
                                // tslint:disable-next-line: prettier
                                '\'' +
                                i +
                                // tslint:disable-next-line: prettier
                                ...
                                item.TRAINING_GUBUN +
                                // tslint:disable-next-line: prettier
                                '\');" class="btn btn-danger btn-xs btn-outline btn-fit">확정</a></td>';
    ...
    // (중간 화면) - 모델 평가 > 모델 확정 처리
    onUpdateTrainingResult(seq: any, DOC_SN: any, STAT_CD: any, TRAINING_GUBUN: any) {
        this.DOC_SN = DOC_SN;
        this.STAT_CD = STAT_CD;
        this.TRAINING_GUBUN = TRAINING_GUBUN;

        if (this.STAT_CD == '30') {
            swal.fire('선택한 문서는 이미 [모델 확정] 처리되었습니다 - 확인하세요.', '', 'warning');
            return;
        }
        ...
        const data = {
            ...
            STAT_CD: '30', // 처리상태 : 30(모델 확정)
        };

        swal.fire({
            title: '[모델 확정] 처리 하시겠습니까?',
            icon: 'question',
            ...
            if (result.value) {
                this.apigatewayService
                    .doPost('recognition/modelEvaluationTrainingResult', data, null)
                    .subscribe(
                        resultData => {
                            if (resultData.code === 200) {
                                swal.fire(
                                    '[모델 확정]이 정상적으로 처리 되었습니다.',
                                    '',
                                    'success'
                                ).then(result => {
                                    this.onSearchOcrDocMstList(this.mstPageNo);
                                    this.onSelectOcrDocTextPaging(this.ocrDocTextPageNo, DOC_SN);
                                });
                            } else {
                    ...
                    if (resultData.data.length > 0) {
                        ...
                        $.each(resultData.data, (index: any, item: any) => {
                            ...
                            tmp +=
                                ...
                                '" data-lightbox="automlTrainingImage"><img style="max-height:300px;" src="' +
                                this.apigatewayService.apiUrl +
                                '/textimagefile/' +
                                item.FILE_FULL_PATH_R.replace(
                                    'C:\\dev_project\\auto_ml\\textimagefile\\',
